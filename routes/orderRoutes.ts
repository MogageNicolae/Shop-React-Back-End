...
router.post('', jsonParser, (req, res) => {
    const clientId: string = req.body.id,
        next = () => OrderModel.find({'cart.clientId': clientId}).exec().then(
            (orderData) => {
                res.json(orderData);
            });
...
router.post('/add', jsonParser, (req, res) => {
    const clientId: string = req.body.id,
        next = () => CartModel.findOneAndDelete({clientId: clientId}).exec().then(
            (cartData) => {
                if (cartData === null) {
                    res.json("Error placing order.");
                    return;
                }
                createNewOrder(req.body, cartData).then(
                    (data) => {
                        res.json(data);
                    });
            });
...
router.post('/:id', jsonParser, (req, res) => {
    const clientId: string = req.body.id,
        next = () => OrderModel.findById({_id: req.params.id}).exec().then(
            (orderData) => {
                res.json(orderData);
            });
    checkToken(req, res, clientId, next);
