...
router.post('/register', jsonParser, async (req, res) => {
    ClientModel.findOne({email: req.body.email}).exec().then(
        (data) => {
            if (data !== null) {
...
router.post('/login', jsonParser, async (req, res) => {
    ...
    ClientModel.findOne({email: bodyEmail, password: req.body.password}).exec().then(
        (data) => {
            if (data) {
                data.token = uuidv4();
...
router.post('/logout', jsonParser, async (req, res) => {
    ClientModel.findOneAndUpdate({token: req.header('Auth')},  { token: '' }).exec().then(
        (data) => {
            if (data === null) {
